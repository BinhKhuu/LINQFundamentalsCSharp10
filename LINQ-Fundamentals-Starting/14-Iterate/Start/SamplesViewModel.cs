namespace LINQSamples
{
    public class SamplesViewModel : ViewModelBase
    {
        ...
        public List<SalesOrder> ForEachQuery()
        {
            ...
            (from s in sales
             let lineTotal = s.LineTotal = s.OrderQty * s.UnitPrice
             select s).ToList();
        ...
        public List<Product> ForEachSubQueryQuery()
        {
            ...
            // Write Query Syntax Here
            (from p in products
             let totalSales = p.TotalSales = sales.Where(s => s.ProductID == p.ProductID)
               .Sum(s => s.LineTotal)
             select p
 ).ToList();
        ...
        /// <summary>
        /// Iterate over each object in the collection and call a sub query to calculate total sales (another collection)
        /// </summary>
        ...
        public List<Product> ForEachQueryCallingMethodQuery()
        {
            ...
            return (from p in products
             let temp = p.TotalSales = CalculateTotalSalesForProduct(p, sales)
             select p).ToList();
